...
const nav = [
  ...
  { label: '生成任务', icon: PhList, to: '/tasks', demo: true },
  { label: '专业工作台', icon: PhVideoCamera, to: '/studio/p-2408-01', demo: true },
  { label: '审核中心', icon: PhCheckSquare, to: '/review', demo: true },
  { label: '导出中心', icon: PhArchiveTray, to: '/exports', demo: true },
  { label: '知识资产', icon: PhBooks, to: '/assets', demo: true },
...
const orgName = computed(() => auth.user?.dept?.deptName || '浙旅智创')
const roleLabel = computed(() => auth.roles.includes('platform_admin') || auth.roles.includes('admin') ? '平台管理员' : auth.roles.includes('org_admin') ? '组织管理员' : '内容成员')
...
onMounted(() => {
  ...
  if (!auth.user) void auth.loadUser()
})
    ...
    <aside class="sidebar" aria-label="主导航">
      ...
      <nav class="nav-list">
        <button v-for="item in nav" :key="item.to" class="nav-item" :class="{ active: route.path === item.to || (item.to !== '/' && route.path.startsWith(item.to.split('/:')[0])) }" type="button" :title="collapsed ? item.label : undefined" @click="navigate(item.to)">
          <component :is="item.icon" :size="20" weight="regular" /><span v-if="!collapsed">{{ item.label }}<small v-if="item.demo" class="nav-demo">演示</small></span>
        </button>
      </nav>
      <div class="sidebar-foot">
        <div class="org-card" :class="{ compact: collapsed }"><span class="avatar avatar--photo">{{ avatarLabel }}</span><span v-if="!collapsed"><strong>{{ orgName }}</strong><small>{{ roleLabel }}</small></span></div>
    ...
    <section class="app-main">
      <header class="topbar">
        ...
        <div class="topbar-actions">
          ...
          <button class="user-menu" type="button" :aria-expanded="userOpen" @click="userOpen = !userOpen; noticeOpen = false"><span class="avatar">{{ avatarLabel }}</span><span class="user-copy"><strong>{{ displayName }}</strong><small>{{ roleLabel }}</small></span></button>
          ...
          <Transition name="panel-pop"><aside v-if="userOpen" class="top-popover user-popover"><button @click="navigate('/settings'); userOpen = false"><PhUserCircle :size="18" />账户与组织</button><button @click="signOut"><PhSignOut :size="18" />退出登录</button></aside></Transition>
        </div>
      </header>
